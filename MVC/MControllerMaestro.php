...
class MControllerMaestro
{
    ...
    */
    ...
    public function dispatch(Request $request, Response $response, MActionDispatcher $dispatcher): Response
    {
        $this->request = $request;
        $this->response = $response;
        $this->action = $dispatcher->getAction();
        mtrace('mcontroller::dispatch = ' . $this->action);
        $this->result = new MResultNull;
        $this->decryptData();
        if (!method_exists($this, $this->action)) {
            throw new HttpNotFoundException($this->request, 'Action ' . $this::class . ':' . $this->action . ' not found!');
        } else {
    ...
    {
        $base = str_replace("Controller", "", str_replace("Controllers","Views", str_replace("\\", "/", lcFirst($this::class))));
        $path = Manager::getBasePath() . '/' . $base . '/' . $viewName;
        $extensions = ['.blade.xml', '.xml', '.blade.php', '.html', '.js', '.php', '.latte'];
        $viewFile = '';
